...
const PriceDisplay = ({ item, priceHeaders }: { item: MenuItem; priceHeaders?: [string, string] }) => {
  if (item.priceDom || item.priceImp) {
    return (
      <div className="flex gap-4 font-body text-base md:text-lg font-semibold text-gold tabular-nums shrink-0 mt-0.5">
        <div className="min-w-[40px] md:min-w-[50px] text-right"><FormattedPrice price={item.priceDom} /></div>
        <div className="min-w-[40px] md:min-w-[50px] text-right"><FormattedPrice price={item.priceImp} /></div>
      </div>
    );
  }
  if (item.priceGlass || item.priceBottle) {
    return (
      <div className="flex gap-4 font-body text-base md:text-lg font-semibold text-gold tabular-nums shrink-0 mt-0.5">
        <div className="min-w-[40px] md:min-w-[50px] text-right"><FormattedPrice price={item.priceGlass} /></div>
        <div className="min-w-[65px] md:min-w-[85px] text-right"><FormattedPrice price={item.priceBottle} /></div>
      </div>
    );
  }
  if (item.price300 || item.price650) {
    return (
      <div className="flex gap-4 font-body text-base md:text-lg font-semibold text-gold tabular-nums shrink-0 mt-0.5">
        <div className="min-w-[40px] md:min-w-[50px] text-right"><FormattedPrice price={item.price300} /></div>
        <div className="min-w-[40px] md:min-w-[50px] text-right"><FormattedPrice price={item.price650} /></div>
      </div>
    );
  }
  if (item.price) {
    return (
      <span className="font-body text-base md:text-lg font-semibold text-gold tabular-nums shrink-0 mt-0.5 whitespace-nowrap min-w-[50px] text-right">
